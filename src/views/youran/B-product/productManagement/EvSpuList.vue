      ...
      <a-form ref="formRef" @keyup.enter.native="searchQuery" :model="queryParam" :label-col="labelCol" :wrapper-col="wrapperCol">
        <a-row :gutter="24">
          ...
          <a-col :lg="6">
            ...
            <a-col :lg="6">
              <a-form-item name="code">
                <template #label><span title="厂家名称">厂家名称</span></template>
                <JInput placeholder="请输入厂家名称" v-model:value="queryParam.supplier"></JInput>
              </a-form-item>
            </a-col>
            <!-- <a-col :lg="6">
            ...
            <!-- <a-col :lg="6">
              <a-form-item name="status">
                <template #label><span title="启用状态">启用状态</span></template>
                <JDictSelectTag type="select" v-model:value="queryParam.status" dictCode="ev_spu_status" placeholder="请选择启用状态" />
              </a-form-item>
            </a-col> -->
            <!-- <a-col :lg="6">
    ...
    <BasicTable @register="registerTable">
      ...
      <template v-slot:bodyCell="{ column, record, index, text }">
        ...
        <template v-if="column.dataIndex==='status'">
          <JDictSelectTag v-model:value="record.status" type="radioButton" dict-code="ev_goods_status" @change="handleEditStatus(record)"></JDictSelectTag>
          <!-- <div style="display: flex;width: 100%;"><JSwitch v-model:value="record.status" :options="['0', '1']" :labelOptions="['上架','下架']"></JSwitch>{{ record.status_dictText }}</div> -->
        </template>
      </template>
    </BasicTable>
    <!-- 表单区域 -->
...
<script lang="ts" name="evSpu-evSpu" setup>
  ...
  import {saveOrUpdate} from './EvSpu.api';
   ...
   * 编辑事件上架和下架
   */
   async function handleEditStatus(record: Recordable) {
    const data = record
    // console.log("---------------",data)
    // data.status = (record.status === '0'?'1':'0')
    // registerModal.value.disableSubmit = false;
    // registerModal.value.edit(data);

    
  await saveOrUpdate(data, true)
      .then((res) => {
        if (res.success) {
          createMessage.success(data.status === '0'?'商品已上架!':'商品已下架!');
        } else {
  ...
  function getTableAction(record) {
    return [
      ...
      //   onClick: handleEditStatus.bind(null, record),
      // },
      {
        label: '编辑',
